...
import { Play, WifiOff, AlertTriangle, Key, Music, CheckCircle2, X } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Listener para erros de cota do GeminiService
    const handleQuotaError = () => setShowQuotaWarning(true);
    window.addEventListener('gemini_quota_error', handleQuotaError);
    ...
    return () => {
      ...
      window.removeEventListener('gemini_quota_error', handleQuotaError);
    };
  }, [refreshProfiles]);
  ...
  return (
    <div className="min-h-screen bg-[#141414] text-white overflow-x-hidden">
      ...
      {showQuotaWarning && isOnline && (
        <div className="fixed top-24 left-4 right-4 md:left-12 md:right-auto md:w-96 bg-red-600/95 backdrop-blur-xl text-white p-5 z-[60] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] flex flex-col gap-4 border border-white/20 animate-in slide-in-from-left-4 duration-500">
          <div className="flex items-start justify-between">
             <div className="flex items-start gap-3">
                <AlertTriangle className="w-6 h-6 flex-shrink-0 text-yellow-300" />
                <div>
                  <h4 className="font-black text-sm uppercase tracking-widest">Cota de IA Esgotada</h4>
                  <p className="text-xs text-white/90 leading-relaxed mt-1">
                    O limite gratuito da API Gemini foi atingido. Configure sua própria chave para resultados em tempo real.
                  </p>
                </div>
             </div>
             <button onClick={() => setShowQuotaWarning(false)} className="text-white/50 hover:text-white transition">
                <X className="w-5 h-5" />
             </button>
          </div>
          <div className="flex flex-col gap-2">
            <button 
              ...
              className="w-full bg-white text-black py-3 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-gray-200 transition active:scale-95 shadow-lg"
            >
              <Key className="w-4 h-4" /> ATUALIZAR API KEY
            </button>
            <p className="text-[9px] text-center text-white/40 uppercase font-bold">Usando dados de reserva por enquanto</p>
          </div>
        </div>
      )}

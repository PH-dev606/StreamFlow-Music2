...
export const addToPlaylist = (profileId: string, playlistId: string, item: MusicItem) => {
  ...
  triggerBackgroundDownload(profileId, item);
};

export const removeFromPlaylist = (profileId: string, playlistId: string, itemId: string) => {
  const playlists = getPlaylists(profileId);
  const updated = playlists.map(pl => {
    if (pl.id === playlistId) {
...
// Smart Background Download
export const triggerBackgroundDownload = (profileId: string, item: MusicItem) => {
  ...
  if (!pending.includes(item.id)) {
    localStorage.setItem(`${KEYS.PENDING_DOWNLOADS}${profileId}`, JSON.stringify([...pending, item.id]));
  }
};
...
export const toggleMyList = (profileId: string, item: MusicItem) => {
  ...
  if (!exists) triggerBackgroundDownload(profileId, item);
};
